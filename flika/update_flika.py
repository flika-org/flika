...
__all__ = ['checkUpdates', 'check_if_installed_via_pip', 'get_pypi_version', 'get_github_version', 
           'path_walk', 'capture', 'updateFlika', 'check_updates_no_gui', 'update_flika_no_gui']
    ...
    try:
        ...
        with urlopen(req, timeout=10) as response:
            if response.status != 200:
                g.alert("PyPI Connection Failed", 
            ...
            if not latest_version:
                g.alert("Version Not Found", "Could not find version information in PyPI response.")
                return None
...
def get_github_version() -> str | None:
    ...
    try:
        ...
        g.alert("Connection Failed",
                f"Cannot connect to flika Repository. HTTP Error: {e}")
        return None
    except URLError as e:
        g.alert("Connection Failed",
        ...
        g.alert("Parsing Error",
                f"Failed to parse pyproject.toml: {e}")
        return None
    except Exception as e:
        g.alert("Unexpected Error",
...
def check_updates_no_gui() -> tuple[bool, str, str | None]:
    ...
    try:
        ...
        if latest_version is None:
            return False, f"{message}\nUnable to determine the latest version from {source}.", None
        ...
        try:
            ...
            if current_version < remote_version:
                ...
                return False, f"{message}\n\nYour version is up to date.", latest_version
                
        except packaging.version.InvalidVersion:
            return False, f"{message}\n\nCannot compare versions due to invalid format.", latest_version
...
def checkUpdates() -> bool:
    ...
    try:
        ...
        g.alert("Update Check Failed", f"An unexpected error occurred while checking for updates: {e}")
        return False
...
def update_flika_no_gui() -> tuple[bool, str]:
    ...
    try:
        ...
        if installed_via_pip:
            try:
                out = check_output([sys.executable, '-m', 'pip', 'install', '-U', '--no-deps', 'flika'], timeout=60)
            ...
            if flika_location.joinpath('.git').exists():
                return False, "This installation of flika is managed by git. Upgrade will not proceed. Use git to upgrade."
                
            extract_location = tempfile.mkdtemp()
            
            try:
                with urlopen('https://github.com/flika-org/flika/archive/master.zip', timeout=30) as response:
                    if response.status != 200:
                        if os.path.exists(extract_location):
                            shutil.rmtree(extract_location)
                        return False, f"Failed to download flika: HTTP status {response.status}"
