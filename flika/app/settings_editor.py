...
class SettingsEditor(BaseDialog):
    ...
    def __init__(self):
        ...
        items.append(
            {
                "name": "random_color_check",
                "string": "Randomly color ROIs",
                "object": random_color_check,
            }
        )
        items.append(
            {"name": "roi_color", "string": "Default ROI Color", "object": roi_color}
        )
        items.append(
            {
                "name": "internal_data_type",
                "string": "Internal Data Type",
                "object": dataDrop,
            }
        )
        items.append(
            {"name": "show_windows", "string": "Show Windows", "object": showCheck}
        )
        items.append(
            {
                "name": "multipleTraceWindows",
                "string": "Multiple Trace Windows",
                "object": multipleTracesCheck,
            }
        )
        items.append(
            {
                "name": "multiprocessing",
                "string": "Multiprocessing On",
        ...
        items.append(
            {
                "name": "nCores",
                "string": "Number of cores to use when multiprocessing",
                "object": nCores,
            }
        )
        items.append(
            {"name": "debug_mode", "string": "Debug Mode", "object": debug_check}
        )
        items.append(
            {
                "name": "default_roi_on_click",
                "string": "Enable default ROI on right click",
                "object": default_roi,
            }
        )
        ...
        def update():
            ...
            g.settings["roi_color"] = (
                roi_color.value() if not random_color_check.isChecked() else "random"
            )
            roi_color.setEnabled(g.settings["roi_color"] != "random")
...
def pencilSettings(pencilButton):
    ...
    items = []
    items.append(
        {"name": "pencil_value", "string": "Pencil Value", "object": pencil_value}
    )
...
def pointSettings(pointButton):
    ...
    items = []
    items.append(
        {"name": "point_color", "string": "Default Point Color", "object": point_color}
    )
    items.append(
        {"name": "point_size", "string": "Default Point Size", "object": point_size}
    )
    items.append(
        {
            "name": "show_all_points",
            "string": "Show points from all frames",
            "object": show_all_points,
        }
    )
    items.append(
        {
            "name": "update_current_points_check",
            "string": "Update already plotted points",
            "object": update_current_points_check,
        }
    )
    items.append(
        {
            "name": "delete_all_points",
            "string": "Delete all points",
            "object": delete_all_points,
        }
    )
...
def rectSettings(rectButton):
    ...
    items.append(
        {
            "name": "rect_width",
            "string": "Default Rectangle Width",
            "object": rect_width,
        }
    )
    items.append(
        {
            "name": "rect_height",
            "string": "Default Rectangle Height",
            "object": rect_height,
        }
    )
