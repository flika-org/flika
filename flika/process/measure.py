...
class Measure(BaseProcess):
    ...
    def gui(self):
        ...
        self.items.append(
            {"name": "active_point", "string": "Active Point: ", "object": active_point}
        )
        self.items.append(
            {"name": "second_point", "string": "Second Point: ", "object": second_point}
        )
        self.items.append(
            {"name": "difference", "string": "Difference: ", "object": difference}
        )
    ...
    def export(self, filename):
        ...
        for j in np.arange(maxj):  # this loops through each row of data
            for i in np.arange(
                len(d)
            ):  # this loops through each mega-column, each of which is composed of 7 mini-columns.
        ...
        g.m.statusBar().showMessage(
            "Successfully saved {}".format(os.path.basename(filename))
        )
