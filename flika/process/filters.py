...
class Gaussian_blur(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        self.items.append(
            {"name": "sigma", "string": "Sigma (pixels)", "object": sigma}
        )
        self.items.append(
            {"name": "norm_edges", "string": "Normalize Edges", "object": norm_edges}
        )
        ...
        if sigma > 0:
            ...
            if len(self.tif.shape) == 3:
                for i in np.arange(len(self.newtif)):
                    self.newtif[i] = skimage.filters.gaussian(
                        self.tif[i].astype(np.float64), sigma, mode=mode
                    )
            elif len(self.tif.shape) == 2:
                self.newtif = skimage.filters.gaussian(
                    self.tif.astype(np.float64), sigma, mode=mode
                )
...
class Difference_of_Gaussians(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "sigma1", "string": "Sigma 1 (pixels)", "object": sigma1}
        )
        self.items.append(
            {"name": "sigma2", "string": "Sigma 2 (pixels)", "object": sigma2}
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
        super().gui()
        self.preview()
    ...
    def __call__(self, sigma1: float, sigma2: float, keepSourceWindow: bool = False):
        ...
        if sigma1 > 0 and sigma2 > 0:
            ...
            if len(self.tif.shape) == 3:
                for i in np.arange(len(self.newtif)):
                    self.newtif[i] = skimage.filters.gaussian(
                        self.tif[i].astype(np.float64), sigma1, mode="nearest"
                    ) - skimage.filters.gaussian(
                        self.tif[i].astype(np.float64), sigma2, mode="nearest"
                    )
            elif len(self.tif.shape) == 2:
                self.newtif = skimage.filters.gaussian(
                    self.tif.astype(np.float64), sigma1, mode="nearest"
                ) - skimage.filters.gaussian(
                    self.tif.astype(np.float64), sigma2, mode="nearest"
                )
            self.newtif = self.newtif.astype(g.settings["internal_data_type"])
        else:
            self.newtif = self.tif
        self.newname = self.oldname + " - Difference of Gaussians ({} {})".format(
            sigma1, sigma2
        )
        return self.end()
    ...
    def preview(self, *args, **kwargs) -> None:
        ...
        if preview:
            if len(g.win.image.shape) == 3:
                ...
                testimage = g.win.image.astype(np.float64)
            if sigma1 > 0 and sigma2 > 0:
                testimage = skimage.filters.gaussian(
                    testimage, sigma1, mode="nearest"
                ) - skimage.filters.gaussian(testimage, sigma2, mode="nearest")
            g.win.imageview.setImage(testimage, autoLevels=False)
        else:
            g.win.reset()
...
class Butterworth_filter(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "filter_order", "string": "Filter Order", "object": filter_order}
        )
        self.items.append(
            {"name": "low", "string": "Low Cutoff Frequency", "object": low}
        )
        self.items.append(
            {"name": "high", "string": "High Cutoff Frequency", "object": high}
        )
        self.items.append(
            {"name": "framerate", "string": "Frame rate (Hz)", "object": framerate}
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
        super().gui()
        if g.win is None:
    ...
    def __call__(
        ...
        if g.settings["multiprocessing"]:
            self.newtif = butterworth_filter_multi(
                filter_order, low / (framerate / 2), high / (framerate / 2), g.win.image
            )
        else:
            self.newtif = np.zeros(
                self.tif.shape, dtype=g.settings.d["internal_data_type"]
            )
            mt, mx, my = self.tif.shape
            b, a, padlen = self.makeButterFilter(
                filter_order, low / (framerate / 2), high / (framerate / 2)
            )
            for i in np.arange(mx):
                for j in np.arange(my):
                    self.newtif[:, i, j] = filtfilt(
                        b, a, self.tif[:, i, j], padlen=padlen
                    )
            ...
            if self.roi is not None:
                if preview:
                    if (low == 0 and high == framerate / 2) or (low == 0 and high == 0):
                        ...
                        g.currentTrace.update_trace_full(
                            roi_index, trace
                        )  # update_trace_partial may speed it up
                else:
...
def butterworth_filter_multi(
    ...
    progress = ProgressBar(
        butterworth_filter_multi_inner,
        data,
        args,
        nThreads,
        msg="Performing Butterworth Filter",
    )
...
def butterworth_filter_multi_inner(
    ...
    status = q_status.get(
        True
    )  # this blocks the process from running until all processes are launched
    if status == "Stop":
        q_results.put(None)
...
class Mean_filter(BaseProcess):
    ...
    def __call__(self, nFrames: int, keepSourceWindow: bool = False):
        ...
        if self.tif.ndim != 3:
            ...
            return
        self.newtif = convolve(
            self.tif, weights=np.full((nFrames, 1, 1), 1.0 / nFrames)
        )
...
class Median_filter(BaseProcess):
    ...
    def __call__(self, nFrames: int, keepSourceWindow: bool = False):
        ...
        if self.tif.ndim < 3:
            g.alert(
                "Median filter requires at least 3 dimensions. %d < 3" % self.tif.ndim
            )
            return
        mx = self.tif.shape[2]
        my = self.tif.shape[1]
...
class Fourier_filter(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        self.items.append(
            {"name": "frame_rate", "string": "Frame Rate (Hz)", "object": frame_rate}
        )
        self.items.append(
            {"name": "low", "string": "Low Cutoff Frequency", "object": low}
        )
        ...
        self.items.append(
            {
                "name": "loglogPreview",
                "string": "Plot frequency spectrum on log log axes",
                "object": loglogPreview,
            }
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
        super().gui()
...
class Boxcar_differential_filter(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> bool:
        ...
        self.items.append(
            {
                "name": "minNframes",
                "string": "Minimum Number of Frames",
                "object": minNframes,
            }
        )
        self.items.append(
            {
                "name": "maxNframes",
                "string": "Maximum Number of Frames",
                "object": maxNframes,
            }
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
    ...
    def __call__(
        ...
        for i in np.arange(maxNframes, len(self.newtif)):
            self.newtif[i] = self.tif[i] - np.min(
                self.tif[i - maxNframes : i - minNframes], 0
            )
    ...
    def preview(self, *args, **kwargs) -> None:
        ...
        if self.roi is not None:
            if preview:
                ...
                (mt, mx, my) = self.tif.shape
                cnt = np.array(
                    [np.array([np.array([p[1], p[0]])]) for p in self.roi.pts]
                )
                ...
                for i in np.arange(maxNframes, mt):
                    newtrace[i] = np.mean(
                        tif[i] - np.min(tif[i - maxNframes : i - minNframes], 0)
                    )
                roi_index = g.currentTrace.get_roi_index(self.roi)
                g.currentTrace.update_trace_full(
                    roi_index, newtrace
...
class Wavelet_filter(BaseProcess):
    ...
    ***Warning!! This function is extremely slow.***
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        preview.setChecked(True)
        self.items.append(
            {"name": "low", "string": "Low Frequency Threshold", "object": low}
        )
        self.items.append(
            {"name": "high", "string": "High Frequency Threshold", "object": high}
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
        super().gui()
        self.roi = g.win.currentROI
...
class Bilateral_filter(BaseProcess):
    ...
    def gui(self, *args, **kwargs) -> None:
        ...
        stoptol.setRange(0, 0.02)
        stoptol.setValue(0.001)
        ...
        self.items.append(
            {"name": "stoptol", "string": "stop tolerance", "object": stoptol}
        )
        self.items.append(
            {"name": "maxiter", "string": "Maximum Iterations", "object": maxiter}
        )
        self.items.append({"name": "preview", "string": "Preview", "object": preview})
        super().gui()
        self.roi = g.win.currentROI
    ...
    def __call__(
        ...
        if self.tif.ndim != 3:
            ...
            return
        if g.settings["multiprocessing"]:
            self.newtif = bilateral_filter_multi(
                soft, beta, width, stoptol, maxiter, g.win.image
            )
        else:
            self.newtif = np.zeros(self.tif.shape)
            mt, mx, my = self.tif.shape
            for i in np.arange(mx):
                for j in np.arange(my):
                    self.newtif[:, i, j] = bilateral_smooth(
                        soft, beta, width, stoptol, maxiter, self.tif[:, i, j]
                    )
...
def bilateral_filter_multi(
    ...
    progress = ProgressBar(
        bilateral_filter_inner, data, args, nThreads, msg="Performing Bilateral Filter"
    )
    if progress.results is None or any(r is None for r in progress.results):
        result = None
    else:
...
def bilateral_filter_inner(
    ...
    soft, beta, width, stoptol, maxiter = (
        args  # unpack all the variables inside the args tuple
    )
    result = np.zeros(data.shape)
    tt, xx, yy = data.shape
    for x in np.arange(xx):
        for y in np.arange(yy):
            result[:, x, y] = bilateral_smooth(
                soft, beta, width, stoptol, maxiter, data[:, x, y]
            )
