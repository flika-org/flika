    ...
    def __call__(self, framerate, show=True, keepSourceWindow=None):
        ...
        if show:
            ...
            if hasattr(w, "timeStampLabel") and w.timeStampLabel is not None:
                return
            w.timeStampLabel = pg.TextItem(
                html="<span style='font-size: 12pt;color:white;background-color:None;'>0 ms</span>"
            )
...
class Background(BaseProcess):
        ...
        self.items.append(
            {
                ...
                "object": background_window,
            }
        )
        self.items.append(
            {"name": "data_window", "string": "Data window", "object": data_window}
        )
    ...
    def __call__(
        self, background_window, data_window, opacity, show, keepSourceWindow=False
    ):
...
class Scale_Bar(BaseProcess):
    ...
    def gui(self):
        ...
        width_pixels.setRange(0.001, 1000000)
        ...
        show = CheckBox()
        if (
            hasattr(w, "scaleBarLabel") and w.scaleBarLabel is not None
        ):  # if the scaleBarLabel already exists
        ...
        self.items.append(
            {
                "name": "width_microns",
                "string": "Width of bar in microns",
                "object": width_microns,
            }
        )
        self.items.append(
            {
        ...
        self.items.append(
            {"name": "font_size", "string": "Font size", "object": font_size}
        )
        self.items.append({"name": "color", "string": "Color", "object": color})
        self.items.append(
            {"name": "background", "string": "Background", "object": background}
        )
        self.items.append(
            {"name": "location", "string": "Location", "object": location}
        )
        self.items.append({"name": "show", "string": "Show", "object": show})
    ...
    def __call__(
        ...
        if show:
            ...
            w.scaleBarLabel = pg.TextItem(
                anchor=anchor,
                html="<span style='font-size: {}pt;color:{};background-color:{};'>{} μm</span>".format(
                    font_size, color, background, width_microns
                ),
            )
            w.scaleBarLabel.setPos(pos[0], pos[1])
            w.scaleBarLabel.flika_properties = {
                item["name"]: item["value"] for item in self.items
            }
            ...
            bar = QtWidgets.QGraphicsRectItem(
                QtCore.QRectF(barPoint, QtCore.QSizeF(width_pixels, int(font_size / 3)))
            )
    ...
    def updateBar(self):
        ...
        if location == "Top Left":
            ...
            w.scaleBarLabel.setPos(
                QtCore.QPointF(w.mx - width_pixels / 2 - textWidth / 2, 0)
            )
        elif location == "Lower Right":
            barPoint = QtCore.QPoint(w.mx - width_pixels, w.my - 1.3 * textHeight)
            w.scaleBarLabel.setPos(
                QtCore.QPointF(
                    w.mx - width_pixels / 2 - textWidth / 2, w.my - textHeight
                )
            )
        elif location == "Lower Left":
            ...
            w.scaleBarLabel.setPos(
                QtCore.QPointF(
                    QtCore.QPointF(width_pixels / 2 - textWidth / 2, w.my - textHeight)
                )
            )
        w.scaleBarLabel.bar.setRect(
            QtCore.QRectF(barPoint, QtCore.QSizeF(width_pixels, textHeight / 4))
        )
    ...
    def preview(self):
        ...
        show = self.getValue("show")
        self.__call__(
            width_microns, width_pixels, font_size, color, background, location, show
        )
