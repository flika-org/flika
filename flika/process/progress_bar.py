    ...
    def run(self):
        ...
        for i in range(nCores):
            ...
            p.processes.append(
                Process(
                    target=p.outerfunc,
                    args=(q_result, q_progress, q_status, child_conn, p.args),
                )
            )
            ...
            if started[i]:
                if not p.stopPressed:
                    self.status_update.emit(
                        "Sending data to process {}/{}".format(i + 1, nCores)
                    )
                    tic = time.time()
        ...
        for i in range(nCores):
            if started[i]:
                if not p.stopPressed:
                    self.status_update.emit(
                        "Starting process {}/{}".format(i + 1, nCores)
                    )
...
class ProgressBar(QtWidgets.QWidget):
    ...
    def __init__(
        self, outerfunc, data, args, nCores, msg="Performing Operations", parent=None
    ):
        ...
        ):  # the exec_() loop doesn't wait for loop.quit when running in spyder for some reason.  This is the workaround
            time.sleep(0.01)
            QtWidgets.QApplication.processEvents()
        self.close()
...
def outer_func():
    ...
    data_blocks = [
        original_data[block_ends[i] : block_ends[i + 1]] for i in np.arange(nCores)
    ]  # each thread will get one element of this list.

    # create the ProgressBar object
    progress = ProgressBar(
        inner_func, data_blocks, args, nCores, msg="Performing my cool inner function"
    )
