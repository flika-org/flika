...
class TestSettings:
    ...
    def test_random_roi_color(self):
        ...
        assert (
            roi1.pen.color().name() != roi2.pen.color().name()
        ), "Random ROI color is the same. This could be a random chance. Run repeatedly."

        g.settings["roi_color"] = "#00ff00"
        roi3 = makeROI("rectangle", [[3, 3], [3, 3]])
        assert (
            roi3.pen.color().name() == "#00ff00"
        ), "ROI color set. all rois are same color"
