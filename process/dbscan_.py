...
class Cluster(BaseProcess):
	...
	def __call__(self, epsilon, minP, minNeighbors=1):
		...
		g.m.statusBar().showMessage('%d clusters found. Plotting Cluster 0...' % len(clusters))
		ch = ConvexHull(clusters[0])
		md = gl.MeshData(vertexes=ch.points, faces=ch.simplices)
		g.m.currentWindow.view.addItem(gl.GLMeshItem(meshdata=md, drawFaces=False, drawEdges=True, color=(0, 255, 0, 255)))
